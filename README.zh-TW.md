...
4）運行`$ pip install -r requirements.txt`5）運行：`$ npm install`6）將Flask App設定到app目錄：`(.venv) $ export FLASK_APP=app`
7) Set the Flask Environment to True for development: `(.venv) $ export FLASK_DEBUG=True`8) 設定 SQLAlchemy 資料庫 URI：`(.venv) $ export SQLALCHEMY_DATABASE_URI=...`, default is `sqlite:///app.db`9）設定SQLAlchemy軌道修改：`(.venv) $ export SQLALCHEMY_TRACK_MODIFICATIONS=True`10) 設定密鑰：`(.venv) $ export SECRET_KEY=********`
11) Run the flask app: `(.venv) $ flask run`12）根據提示開啟Web介面
