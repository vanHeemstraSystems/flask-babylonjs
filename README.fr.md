...
4) Courir`$ pip install -r requirements.txt`5) Descendez dans le`app`annuaire:`$ cd app`et exécutez :`$ npm install`6) Remontez dans le`flask_app`annuaire:`$ cd ../`7) Définissez l'application Flask dans le répertoire des applications :`(.venv) $ export FLASK_APP=app`8) Définissez l'environnement Flask sur True pour le développement :`(.venv) $ export FLASK_DEBUG=True`9) Définissez l'URI de la base de données SQLAlchemy :`(.venv) $ export SQLALCHEMY_DATABASE_URI=...`, la valeur par défaut est`sqlite:///app.db`10) Définir les modifications de la piste SQLAlchemy :`(.venv) $ export SQLALCHEMY_TRACK_MODIFICATIONS=True`11) Définir la clé secrète :`(.venv) $ export SECRET_KEY=********`12) Exécutez l'application Flask :`(.venv) $ flask run`13) Ouvrez l'interface Web comme vous y êtes invité
14) Utiliser`CTRL+c`pour quitter le serveur Web.
15) Vous pouvez également exécuter l'interface de ligne de commande flask :`(.venv) $ flask shell`16) Exécutez toutes les commandes du flacon : >>>
17) Utiliser`exit()`pour quitter l'interface de ligne de commande.
